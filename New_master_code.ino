...
void moverServo(int angulo)    // Recogemos las variables PIN y ANGULO en cada llamada 
{
                      // Declaramos la variable float para recoger los resultados de la regla de tres
   degree=(angulo*17/18)+65;   // Calculamos el ancho del pulso aplicando la regla de tres
   if (degree>240){
    degree=240;
   }
   }
...
void setup() {
 ...
 TCCR1A = 0;
  TCCR1B = 0;
  TCCR1B = 0b00000001;      // no scaling 62.5ns period (p. 173 datasheet)
  TCCR1B |= 0b00001000;     // set WGM12 bit to turn on CTC (Clear Timer Compare) mode
  OCR1A = 160;              // 10us interrupt: 62.5ns * 160 = 10us
  TIMSK1 = 0b00000010;      // set OCIEA bit to enable interrupt on match of OCR1A value (p. 184)
...
for(int j = 0; j <= 300; j+= 5)  // goes from 0 degrees to 180 degrees 
  {      // in steps of 1 degree 
     ...
     moverServo(pos);               // tell servo to go to position in variable 'pos' 
  ...
  moverServo(posFinal); //muevo servo
  oldDistance=minDist;
Pos1 =180;
Pos2 =0;
minDist=400;
...
ISR (TIMER1_COMPA_vect) {
  count++;                  // increment counter every 10us interrupt
  if (count==(degree)) {
    digitalWrite(TurretPin, LOW);
  }
  if (count == 2200) {    // 22ms period for jitter free operation
    count=0;
